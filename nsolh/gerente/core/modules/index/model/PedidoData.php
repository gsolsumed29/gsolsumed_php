...
class PedidoData {
	...
	public static function GetPedido($fact_num,$status,$co_ven){
			/// Metodo para consultar todos los datos y mostrar las tablas		
		$co_sucu=$_SESSION['co_alma'];

		$sql ="SELECT p.fact_num, p.saldo, p.fec_emis,p.fec_venc, p.co_tran, p.forma_pag,
		p.tot_bruto, p.tot_neto, p.iva, p.moneda, p.status, p.contrib,c.cli_des,c.email,c.telefonos,c.direc1,t.des_tran,v.ven_des FROM pedidos p 
		INNER JOIN almacen a on p.co_sucu = a.co_alma 
		INNER JOIN clientes c on p.co_cli = c.co_cli 
		INNER JOIN vendedor v on p.co_ven = v.co_ven 
	...
	public static function GetRenglonPedido($fact_num,$co_ven){
		/// Metodo para consultar todos los datos y mostrar las tablas
		$co_sucu=$_SESSION['co_alma'];
		$sql ="SELECT  a.co_art AS co_art, pg.total_art AS cantidad,a.art_des,a.prec_vta1 from Ped_reng pg
