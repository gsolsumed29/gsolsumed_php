...
class EmpresaData {
	...
	public function updateCorreo(){
		$sql = "UPDATE empresa_email SET text='$this->text', smtp='$this->smtp', password='$this->password', host='$this->host',port='$this->port'  WHERE co_emp='001';";
		//echo $sql;
		$query = Executor::doitEx($sql);
	}
	...
	public static function getAllDatosCorreo(){
		$sql = "SELECT ee.text,ee.smtp,ee.password,ee.host,ee.port,e.email,e.email_ventas,e.email_cobros  FROM empresa_email ee 
		INNER JOIN empresa e ON ee.co_emp = e.co_emp WHERE ee.co_emp='001' ORDER BY ee.co_emp ASC";	
		//echo $sql;
		$query = Executor::doitAr($sql);
		//print_r($query);
		$e=count($query);
