...
class PagoData {
	...
	public function add(){		
		...
		$data=$this->ultima();
		$this->co_pago=$data['co_pago'];
		$co_ven =$_SESSION['identidad'];
		//var_dump($this->$data);
		$sql = "INSERT INTO ".self::$tablename." (co_pago, co_cli, co_ven, fec_emis, monto_cob, cob_num,status) ";
		$sql .= "VALUES ('$this->co_pago','$this->co_cli','$co_ven','$this->fec_emis',$this->monto_cob,'$this->co_pago',1)";		
		//echo $sql;	
		Executor::doitEx($sql);		
	}
	...
	public static function ultima(){		
			...
			$sql = "SELECT  MAX(co_pago)+1 as co_pago FROM ".self::$tablename;
			//echo $sql;
