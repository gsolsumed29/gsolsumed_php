...
// ========================================
// ========================================
// FUNCIÓN PARA CARGAR VISITAS DE UN DÍA ESPECÍFICO (CORREGIDA)
// ========================================
function loadDailyVisits(date) {
    console.log('Cargando visitas para:', date);
    ...
    // Ocultar/deshabilitar botón de exportar mientras carga
    var $exportBtn = $('#exportPDFBtn');
    $exportBtn.prop('disabled', true);
    $exportBtn.off('click'); // Remover eventos anteriores
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success && response.data && response.data.length > 0) {
                var visitas = response.data;
                
                // Renderizar tabla
                renderDailyVisitsTable(visitas, dateForAPI);
                
                // Calcular resumen
                var total = visitas.length;
                var visitados = visitas.filter(v => v.visit_status === 'visitado').length;
                var noVisitados = visitas.filter(v => v.visit_status === 'no_visitado').length;
                var pendientes = visitas.filter(v => v.visit_status === 'pendiente' || !v.visit_status).length;
                
                var resumenHtml = `
                    <div class="col-md-12">
                        <div class="d-flex justify-content-start gap-2 flex-wrap">
                            <span class="badge bg-secondary p-2" style="font-size: 14px;">Total: ${total}</span>
                            <span class="badge bg-success p-2" style="font-size: 14px;">Visitados: ${visitados}</span>
                            <span class="badge bg-danger p-2" style="font-size: 14px;">No visitados: ${noVisitados}</span>
                            <span class="badge bg-warning text-dark p-2" style="font-size: 14px;">Pendientes: ${pendientes}</span>
                        </div>
                    </div>
                `;
                
                $('#selectedDateSummary').html(resumenHtml);
                
                // Habilitar botón de exportar y configurar evento
                $exportBtn
                    .prop('disabled', false)
                    .on('click', function(e) {
                        e.preventDefault();
                        ...
                        // Llamar función de exportar
                        generateVisitsPDF(visitas, date, resumen);
                    });
                    
                console.log('Botón de exportar habilitado y configurado');
            ...
            $('#dailyVisitsTableBody').html(`
                <tr>
                    <td colspan="6" class="text-center py-4 text-danger">
                        ...
                        <p>${error}</p>
                        <small class="text-muted">Revise la consola para más detalles</small>
                    </td>
                </tr>
            `);
...
// ========================================
// FUNCIÓN PARA GENERAR PDF DE VISITAS DIARIAS (MEJORADA)
// ========================================
function generateVisitsPDF(visitas, fecha, resumen) {
    console.log('Generando PDF con:', {visitas, fecha, resumen});
    
    try {
        // Verificar que jsPDF está disponible
        if (typeof window.jspdf === 'undefined') {
            toastr.error('Error: Librería jsPDF no está cargada');
        ...
        doc.text('Reporte de Visitas Diarias', 14, 20);
        
        // Título de la tabla detallada (ahora más arriba)
        doc.setFontSize(12);
        doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
        doc.text('Detalle de Visitas', 14, 35);
        
        // Preparar datos para la tabla detallada
        var tableData = visitas.map(function(visita, index) {
            // Formatear hora
            var hora = 'N/A';
            if (visita.inicio) {
            ...
            // Cliente
            var cliente = visita.cliente_nombre || visita.cliente || 'N/A';
            var codigo = visita.cliente_codigo || visita.co_cli || '-';
            
            // Evento
            var evento = visita.titulo || visita.descripcion || 'Sin título';
            
            // Observaciones
            var obs = visita.observations || visita.observaciones || '-';
        ...
        doc.autoTable({
            startY: 40, // Ahora comienza más arriba
            head: [['#', 'Cliente', 'Evento', 'Hora', 'Observaciones']],
            body: tableData,
            theme: 'striped',
            headStyles: {
        ...
        for (var i = 1; i <= pageCount; i++) {
            ...
            doc.text(
                'Este reporte muestra las visitas programadas y su estado de ejecución.',
                14,
        ...
        // Guardar el PDF
        var nombreArchivo = `visitas_${moment(fecha).format('YYYY-MM-DD')}.pdf`;
        ...
        toastr.error('Error al generar el PDF: ' + error.message);
    }
}
        ...
        visitas.forEach(function(visita, index) {
            ...
            html += `
                <tr class="visit-row" data-event-id="${visita.event_id || visita.id}" style="cursor: pointer;">
                    ...
                    <td class="align-middle text-center d-none">${statusBadge}</td>
