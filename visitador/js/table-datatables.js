...
function cargarTablaArticulos(){
  ...
  columnDefs: [
      ...
      },
      {
          ...
          className: 'text-center',
          render: function (data, type, full, meta) {
              const desCol = full['lin_des'] || 'Sin marca';
              return `
...
function cargarTablaClientesCandidatos() {
    ...
    if (dt_basic_table_clientes_visitas.length) {
            ...
            $('body').append(`
                <div class="modal fade" id="modalFichaCandidato" tabindex="-1" aria-labelledby="modalFichaCandidatoLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modalFichaCandidatoLabel">Vista Rápida del Candidato</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <p><strong>Nombre:</strong> <span id="ficha-nombre-completo"></span></p>
                                <p><strong>RIF:</strong> <span id="ficha-rif"></span></p>
                                ...
                                <p><strong>Dato1:</strong> <span id="ficha-dato1"></span></p>
                                <p><strong>Dato2:</strong> <span id="ficha-dato2"></span></p>
                                <p><strong>Dato3:</strong> <span id="ficha-dato3"></span></p>
                                <p><strong>Dato4:</strong> <span id="ficha-dato4"></span></p>
                                <p><strong>Dato5:</strong> <span id="ficha-dato5"></span></p>
                                <p><strong>Foto:</strong> <span id="ficha-foto"></span></p>
                            </div>
                            <div class="modal-footer">
                                <a href="#" id="ficha-boton-visitar" class="btn btn-primary">Visitar Página Completa</a>
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                            </div>
                        </div>
                    </div>
      ...
      // --- LÓGICA PARA MOSTRAR EL MODAL DE DETALLES (LONG-PRESS Y CLICK) ---
      function mostrarFichaClienteCandidato(data) {
        // Rellenamos los campos del modal con los datos
        $('#ficha-nombre-completo').text(data.cli_des || 'N/A');
        $('#ficha-rif').text(data.rif || 'N/A');
        $('#ficha-telefonos').text(data.telefonos || 'N/A');
        ...
        $('#ficha-dato3').text(data.co_cli || 'N/A'); // Usando co_cli como dato3
        $('#ficha-dato4').text(data.dato4 || 'N/A');
        $('#ficha-dato5').text(data.dato5 || 'N/A');
        $('#ficha-foto').text(data.foto || 'N/A');

        // Actualizar el enlace del botón "Visitar Página Completa"
        $('#ficha-boton-visitar').attr('href', `index.php?view=candidato_1&co_cli=${data.co_cli}&cand_des=${encodeURIComponent(data.cli_des)}&foto=${data.foto}`);
        ...
        // --- INICIALIZACIÓN DATATABLE CORREGIDA ---
        var dt_adv_filter = dt_basic_table_clientes_visitas.DataTable({
            data: arrayClientes,
            
            // DEFINICIÓN CORRECTA DE COLUMNAS según lo que veo en la imagen
            columns: [
                { data: 'responsive_id' },      // 0 - Control responsive
                { data: 'co_cli' },              // 1 - Código (J085031723)
                { data: 'cli_des' },              // 2 - Razón Social (FA FARMACIA...)
                { data: 'rif' },                  // 3 - RIF (J085031723)
                { data: 'telefonos' },            // 4 - Teléfonos (4145418875)
                { data: 'email' },                // 5 - Email (N/A)
                { data: 'dato2' },                // 6 - Estado (🔒 en la imagen)
                { data: 'dato3' },                // 7 - Dato adicional
                { data: 'direc1' },               // 8 - Dirección
                { data: 'dato1' },                // 9 - Dato1
                { data: 'foto' },                 // 10 - Foto
            ...
            columnDefs: [
                ...
                // Columna 1: CÓDIGO (visible) - CORREGIDO
                {
                    targets: 1,
                    title: 'CÓDIGO',
                    width: '10%',
                ...
                // Columna 2: RAZÓN SOCIAL (con avatar) - CORREGIDO
              // Columna 2: RAZÓN SOCIAL (con avatar Y LINK CLIQUEABLE)
{
  targets: 2,
  title: 'RAZÓN SOCIAL',
  responsivePriority: 1,
  ...
  render: function (data, type, full, meta) {
      ...
      var needsTooltip = $name.length > 50;
      var displayName = needsTooltip ? $name.substring(0, 47) + '...' : $name;
      ...
      var $output;
      if ($image) {
          $output = '<img src="' + $image + '" alt="Avatar" width="32" height="32">';
      } else {
      ...
      // VERSIÓN CON LINK - El nombre es cliqueable
      return '<div class="d-flex justify-content-left align-items-start">' +
             ...
             '<span class="cliente-name-link text-primary fw-bold" style="cursor: pointer;">' + displayName + '</span>' + // LINK AQUÍ
             '</h6>' +
             ...
             '</div>';
                ...
                // Columna 4: TELÉFONOS
                {
                    ...
                    render: function(data, type) {
                        if (type === 'display' && data) {
                            var tel = data.toString();
                            if (tel.length > 12) {
                                return '<span title="' + tel + '">' + tel.substring(0, 10) + '...</span>';
                            }
                        }
                        return data || '';
                    }
                ...
                // Columna 5: EMAIL
                {
                    ...
                    render: function(data, type) {
                        if (type === 'display' && data && data !== 'N/A') {
                            ...
                            if (email.length > 20) {
                                return '<span title="' + email + '">' + email.substring(0, 17) + '...</span>';
                            }
                        }
                        return data || '';
                    }
                ...
                // Columna 6: ESTADO (o dato2)
                {
                    ...
                    render: function(data, type) {
                        if (type === 'display') {
                            // Si es un candado o similar, mostrarlo como icono
                            return '<span class="badge bg-light-secondary">' + (data || 'N/A') + '</span>';
                        }
                        return data;
                    }
                },
                
                // Columnas adicionales (ocultas inicialmente)
                {
                    targets: [7, 8, 9, 10], // dato3, direc1, dato1, foto
                    visible: false
                },
                
                // Columna 11: ACCIONES (4 botones como en cuentas por cobrar)
                {
                    targets: -1,
                    title: 'ACCIONES',
                    width: '15%',
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        var telefonos = (full['telefonos'] || '').trim().replace(/'/g, "\\'");
                        var foto = full['foto'] || '';
                        
                        return (
                            '<div class="d-flex align-items-center justify-content-center col-actions" style="gap: 8px;">' +
                            
                            // 1. BOTÓN OJO - Vista Rápida
                            '<a href="javascript:void(0);" class="btn-ver-ficha" data-row-index="' + rowIndex + '" data-bs-toggle="tooltip" data-bs-placement="top" title="Vista Rápida">' +
                            feather.icons['eye'].toSvg({ class: 'font-medium-2 text-body' }) +
                            '</a>' +
                            
                            // 2. BOTÓN USUARIO - Ver Candidato
                            '<a href="index.php?view=candidato_1&co_cli=' + co_cli + '&cand_des=' + encodeURIComponent(cli_des) + '&foto=' + foto + '" data-bs-toggle="tooltip" data-bs-placement="top" title="Visitar candidato">' +
                            feather.icons['map'].toSvg({ class: 'font-medium-2 text-info' }) +
                            '</a>' +
                    ...
                    buttons: [
                        {
                            ...
                            text: feather.icons['file'].toSvg({ class: 'font-small-4 me-50' }) + ' Excel',
                            className: 'dropdown-item',
                            exportOptions: { columns: [1, 2, 3, 4, 5, 6] } // Solo columnas visibles
                        },
                        {
                            extend: 'pdf',
                            text: feather.icons['clipboard'].toSvg({ class: 'font-small-4 me-50' }) + ' Pdf',
            ...
            responsive: {
                details: {
                    display: $.fn.dataTable.Responsive.display.modal({
                        header: function (row) {
                            var data = row.data();
                            return 'Detalles de: ' + (data.cli_des || 'Candidato');
                        }
                    }),
                    type: 'column',
                    renderer: function (api, rowIdx, columns) {
                        var data = $.map(columns, function (col, i) {
                            if (col.title === 'ACCIONES' || col.columnIndex === 0) {
                                return '';
                            }
                            return '<tr><td><strong>' + col.title + ':</strong></td><td>' + col.data + '</td></tr>';
                        }).join('');
                        return data ? $('<table class="table table-sm table-bordered"/>').append('<tbody>' + data + '</tbody>') : false;
                    }
                }
            },
        ...
        // --- EVENTOS ---
        dt_basic_table_clientes_visitas.on('click', '.btn-ver-ficha', function(e) {
            e.preventDefault();
            var rowIndex = $(this).data('row-index');
            var rowData = dt_adv_filter.row(rowIndex).data();
            if (rowData) {
                mostrarFichaCandidato(rowData);
            }
        });

        window.mostrarFichaCandidato = function(data) {
            $('#ficha-nombre-completo').text(data.cli_des || 'N/A');
            $('#ficha-rif').text(data.rif || 'N/A');
            $('#ficha-telefonos').text(data.telefonos || 'N/A');
            $('#ficha-email').text(data.email || 'N/A');
            ...
            const visitUrl = `index.php?view=candidato_1&co_cli=${data.co_cli}&cand_des=${encodeURIComponent(data.cli_des)}&foto=${data.foto}`;
        ...
        dt_basic_table_clientes_visitas.on('draw.dt', function() {
        ...
        // --- EVENTO CLICK EN EL NOMBRE (RAZÓN SOCIAL) ---
dt_basic_table_clientes_visitas.on('mousedown touchstart', '.cliente-name-link', function(e) {
  e.preventDefault();
  const $link = $(this);
  const rowData = dt_adv_filter.row($link.closest('tr')).data();
  if (rowData) {
      mostrarFichaClienteCandidato(rowData);
  }
});

// --- EVENTO LONG-PRESS (mantener presionado) - OPCIONAL ---
let pressTimer;
let isLongPress = false;

dt_basic_table_clientes_visitas.on('mousedown touchstart', '.cliente-name-link', function(e) {
  // Para long-press, solo en dispositivos táctiles
...
function cargarDataArticulos($categoria){
  ...
  $.ajax({
    type: "GET",
    url: '../admin/index.php?action=articulos&tipo=1&accion=99&c=ArticuloData&a=59&t=art&filtro='+$categoria, 
}).done(function(data) { 
  var cadena = JSON.stringify(data);
  //console.log(data);
