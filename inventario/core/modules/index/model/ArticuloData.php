...
class ArticuloData {
	...
	public static function getAllDatos(){
		...
		$co_sub = $_SESSION['almacen'];
		$sql ="SELECT a.prec_vta1, a.prec_vta2, a.prec_vta3,a.co_art,a.uni_venta,a.ult_cos_om, a.ult_cos_un, a.cos_pro_un, a.prec_vta4, a.co_art,
		 a.art_des, (st.stock_act - st.stock_com) stock_act,suni_venta,
	...
	public static function getAllDataFiltradaArticulos($filtro){
		...
		if($filtro =='NO'){
			if($tipo ==1){
				$sql ="SELECT a.mont_comi as comi,
				case when tipo_imp = '1' then (prec_vta1) else prec_vta1 end precio1,
			case when tipo_imp = '1' then (prec_vta2) else prec_vta2 end precio2,
			   ...
			   AND a.procedenci in ('02') ORDER BY a.co_art ASC";	
			}else{
				$sql ="SELECT a.mont_comi as comi,
				 case when tipo_imp = '1' then (prec_vta3) else prec_vta3 end precio1,
			  case when tipo_imp = '1' then (prec_vta4) else prec_vta4 end precio2,
				 a.co_art,a.uni_venta,a.ult_cos_om, a.ult_cos_un, a.cos_pro_un, a.prec_vta4, a.co_art,
				a.art_des, (st.stock_act - st.stock_com) stock_act,uni_venta as suni_venta,
			   case when a.tipo_imp = 1 then t.porc_vent else t.porc_cxs end as impuesto, a.tipo_imp
			   FROM ".self::$tablename." a 
		...
		$data= $ObjetoPedido->tasa2(); //bcv -> parelelo
		$tasa=$data['tasa'];
		//echo $sql; //tabla articulos
		$query = Executor::doitAr($sql);	
		$e=count($query);		
		if($e>=1){
			$array = array();
			...
			$array[$cnt]->tipo_imp = $r['tipo_imp'];	
			$array[$cnt]->stock_act = number_format($r['stock_act'], 2, ',', '.');			
			$array[$cnt]->prec_vta1 =$r['precio1'];
			$array[$cnt]->prec_vta2 =$r['precio1']*$tasa;
			$cnt++;
		}
		return $array;
		}else{
