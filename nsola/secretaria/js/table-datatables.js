function cargarTablaSolicitudes(){
  var dt_basic_table_solicitudes = $('.datatables-basic-solicitudes');   
   
  if (dt_basic_table_solicitudes.length) {
    
    let dataSolicitudes =  $('.dataSolicitudes').val();
    //console.log(dataArticulos);
    let arraySolicitudes = "";
    arraySolicitudes = JSON.parse(dataSolicitudes);
    var dt_basic = dt_basic_table_solicitudes.DataTable({
     data : arraySolicitudes,
      columns: [    
      ...
      columnDefs: [
     ...
     
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var $estatus = {
              1: { title: 'CONSTANCIA DE TRABAJO', class: 'badge-light-success' }
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            return (
              ...
              '<a class="me-25" href="index.php?view=rptCTrabajoSolicitud&id='+full['id']+'&fechaEmision='+full['fechaEmision']+'&cod_emp='+full['cod_emp']+'" data-bs-toggle="tooltip" data-bs-placement="top" title="Ver">' +
...
if ($('.personalRegistrado').length) {
  ...
  gainedChart.render();
  //
  cargarEstadisticas('.cantidadPersonalRegistrado',1,'.cantidadPersonalRegistrado');
  //
}
...
if ($('.factura').length) {  
 ...
 if (btnCart.length) {
  ...
  $('.aprobarSolicitud').on('click', function (e) {
    e.preventDefault();
    Swal.fire({
       title: '¿Deseas Aprobar?',
          text: "Tenga en cuenta que aprobará la emisión de este documento, esta acción habilitará al empleado para la descarga en formato PDF del mismo.",
          icon: 'warning',
          ...
          if (result.isConfirmed) {
            ...
            if (($idSolicitud != '') ) { 
               ...
               var tipo = 1;
               var accion = 3; // actualizar estatus
               var datos =1;
                   $.ajax({
                       url: '../admin/index.php?action=personal', 
                       type:'POST',
                       data:{idSolicitud:$idSolicitud,tipo:tipo,accion:accion,datos:datos},
                       success:function(response){
                      //alert(response);
                           ...
                           if(response == 1){ 
                            ...
                            redireccionar('index.php?view=solicitudes&filtro=1');
                           } 
                           if(response == 2){
  ...
  $('.denegarSolicitud').on('click', function (e) {
    e.preventDefault();
    Swal.fire({
       title: '¿Deseas Denegar?',
          text: "Tenga en cuenta que denegará la emisión de este documento, esta acción imposibilitará de manera definitiva la emisión del formato PDF.",
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
                      ...
                      //alert(response);
                           ...
                           if(response == 1){ 
                             ...
                             Swal.fire({
                               icon: 'success',
                               title: 'Bien...',
                               text: 'Solicitud denegada exitosamente.'
...
function cargarDataSolicitudes($tipo,$estatus){
  if ($('#dataSolicitudes').length) {
  var $tipoSolicitud = $('.tipoSolicitud').val();
  var $estatus = $('.estatus').val();
  $.ajax({
    type: "GET",
    url: '../admin/index.php?action=combos&c=NominaData&a=99&t=jm_solicitudes_personal&tipoSolicitud='+$tipoSolicitud+'&estatus='+$estatus, 
}).done(function(solicitudes) { 
  var cadena = JSON.stringify(solicitudes);
  $('.dataSolicitudes').attr("value",cadena);
...
function cargarEstadisticas($card, $filtro,$label) {
  //console.log($combo);
  $.ajax({
    type: "GET",
    url: '../admin/index.php?action=personal&tipo=1&accion=2&datos=99&c=PersonalData&a=1&t=personal&d=' +$filtro
