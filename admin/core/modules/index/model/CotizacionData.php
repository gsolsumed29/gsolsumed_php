...
$date = strftime("%Y-%m-%d %H:%M:%S", time());	
		...
		$sql .= "VALUES ('$this->fact_num','$this->fiscal',$saldo,cast(getdate() as date),cast(getdate() as date),'$this->co_cli','$this->co_ven','$this->co_tran',
		'$this->forma_pag',$total_bruto,$total_neto,$iva,'$this->co_us_in',cast(getdate() as date),'$this->co_sucu','$this->tasa',
		'$this->co_mone',16.00,8.00,31.00,'0',1)";	
		//echo $sql;
		//echo "mont_comi".$this->dato1;
		Executor::doitEx($sql);
	...
	public static function GetPedido($fact_num,$status){
		...
		p.tot_bruto, p.tot_neto, p.iva, p.moneda, p.status, p.contrib,c.cli_des,c.email,c.telefonos,c.direc1,t.des_tran,v.ven_des FROM cotiz_c p 
		INNER JOIN almacen a on p.co_sucu = a.co_alma 
		INNER JOIN clientes c on p.co_cli = c.co_cli 
		INNER JOIN vendedor v on p.co_ven = v.co_ven 
	...
	public static function GetRenglonPedido($fact_num){
		...
		if($e>=1){
			...
			$array[$cnt]->dato3 =  (float)number_format($r['prec_vta1'], 2, ',', '.');		
			$cnt++;
		}
		return $array;
		}else{
