...
public static function getAllCuentasPorCobrarDetallesGerente($filtro, $co_ven,$estado) {
	...
	public static function getFacturasInventarioDespachosAlmacen($filtro, $filtro2, $verificador) {
		...
		// 1. Filtro por Persona (Vendedor/Ocupante)
		$filtroPersona = ($verificador != 'NO') ? " AND d.co_ven = '$verificador' " : '';
	
		// 2. Filtro por Fecha (Optimizado para SQL Server 2014)
		if ($filtro != 'NO') {
			// Usamos el filtro de rango directamente
			$filtroFecha = " AND d.fecha BETWEEN '$filtro' AND '$filtro2' ";
		} else {
			// Si no hay filtro, usamos el mes actual de forma eficiente para el índice
			$inicioMes = date('Y0401');
			$finMes = date('Ym01', strtotime('+1 month'));
			$filtroFecha = " AND d.fecha >= '$inicioMes' AND d.fecha < '$finMes' ";
		}
	
		// 3. Consulta SQL Optimizada (Sin los LIKE pesados)
		$sql = "SELECT 
					f.fec_emis,
					f.fact_num,
					CAST(f.fact_num AS VARCHAR(20)) AS serie_fact_num,
					d.campo1,
					c.cli_des,
					ISNULL(NULLIF(CAST(f.dir_ent AS VARCHAR(MAX)), ''), 
						ISNULL(NULLIF(CAST(c.dir_ent2 AS VARCHAR(MAX)), ''), c.direc1)) AS direc1,
					c.co_cli,
					c.ciudad,
					c.zip,
					c.telefonos,
					...
					'0' AS EstadoRegistro -- <--- VALOR FIJO ENVIADO PARA GANAR VELOCIDAD
				FROM 
					jm_despacho d WITH (NOLOCK)
					INNER JOIN factura f WITH (NOLOCK) ON f.fact_num = d.fact_num 
					INNER JOIN clientes c WITH (NOLOCK) ON f.co_cli = c.co_cli 
					INNER JOIN jm_personas p WITH (NOLOCK) ON d.co_ven = p.co_ocupa 
					INNER JOIN zona z WITH (NOLOCK) ON z.co_zon = c.co_zon 
					INNER JOIN transpor t WITH (NOLOCK) ON t.co_tran = f.co_tran 
					INNER JOIN vendedor v WITH (NOLOCK) ON f.co_ven = v.co_ven
				WHERE 
					d.estatus IN (1, 2, 3) 
					AND f.campo5 <> '1'
					$filtroFecha  
		...
		if ($e >= 1) {
			...
			foreach ($query as $r) {                
				...
				// --- Lógica de Formateo de Número ---
				$numeroOriginal = trim($r['serie_fact_num']);
				$numeroFormateado = $numeroOriginal; 
	
				if (substr($numeroOriginal, 0, 1) === '5' && strlen($numeroOriginal) == 8) {
					$numeroFormateado = 'NF' . substr($numeroOriginal, 2);
				}
				...
				$array[$cnt]->dato5 = trim($r['estatus']);
				$array[$cnt]->dato6 = trim($r['persona_des']);
				$array[$cnt]->dato7 = $numeroFormateado;
	
				$array[$cnt]->EstadoRegistro = $r['EstadoRegistro'];
...
public function getMetasIndicadoresRotacion($co_ven, $co_zona, $finicio, $ffinal, $indicador) {
    ...
    // 2. Nueva Construcción SQL
    // Enfoque: Detalle de Facturación con Costo y Procedencia
    $sql = "SELECT 
                v.ven_des,
                pr.des_proc,
                SUM(r.total_art - r.total_dev) AS total_art
            FROM factura AS f
            INNER JOIN reng_fac AS r ON f.fact_num = r.fact_num
            INNER JOIN art AS a ON r.co_art = a.co_art
            INNER JOIN clientes cl ON f.co_cli = cl.co_cli
            INNER JOIN zona z ON cl.co_zon = z.co_zon
            LEFT JOIN vendedor AS v ON f.co_ven = v.co_ven
            LEFT JOIN proceden AS pr ON a.procedenci = pr.cod_proc
            WHERE f.anulada = 0 and 
               MONTH(fec_emis) = $periodo_ref AND YEAR(fec_emis) = $anio_ref ";

    // 3. Filtros Dinámicos (Adaptados a la nueva estructura)
    
    // Filtro por Vendedor
    if($co_ven != 'todos' && !empty($co_ven)) {
        // Nota: Si $co_ven viene como string 'V2', se usa tal cual.
        // Si el nuevo enfoque requiere un array IN ('V1', 'V2'), se podría ajustar aquí.
        // Asumimos string simple basado en el código anterior.
        $sql .= " AND f.co_ven = '$co_ven'";
    }

  
    // 4. Agrupación y Orden
    $sql .= " GROUP BY v.ven_des, pr.des_proc
              ORDER BY des_proc ASC";

    // Debug: Descomentar si necesitas verificar la consulta
     //echo $sql; 
    ...
    if($e >= 1) {
        ...
        foreach($query as $r) {
            ...
            // Nota: Es probable que debas agregar estas propiedades a la clase FacturaData
            // si no existen: $ven_des, $des_proc, $total_art.
            
            $array[$cnt]->responsive_id = "";
            $array[$cnt]->ven_des = $r['ven_des'];         // Nombre del vendedor
            $array[$cnt]->des_proc = $r['des_proc'];       // Descripción de procedencia
            $array[$cnt]->total_art = (float)$r['total_art']; // Monto neto calculado

            
            // Opcional: Mantener compatibilidad con el objeto anterior si es necesario
            // $array[$cnt]->monto_facturado = (float)$r['total_art']; 
...
public static function getDataFacturas($mesParametro) {
    ...
    $sql = "
			SELECT
				...
				-- LÓGICA DE PRIORIDAD PARA EL ORDENAMIENTO
				-- Fiscalizables (< 5M) = Prioridad 1
				-- No Fiscalizables (>= 5M) = Prioridad 2
				CASE 
					WHEN c.fact_num < 5000000 THEN 1 
					ELSE 2 
				END AS prioridad_fiscal,
				...
				c.moneda,
				c.tasa,
				CASE WHEN c.moneda = 'USD' THEN c.saldo ELSE 0 END AS saldo_usd,
				CASE WHEN c.moneda != 'USD' OR c.moneda IS NULL THEN c.saldo ELSE 0 END AS saldo_bs,
				...
				c.anulada,
				c.status,
				c.campo1, c.campo2, c.campo3, c.campo4, c.campo5, c.campo6,
				...
				-- Solo aplica retención si es fiscalizable (< 5M) y cumple las otras condiciones
				CASE 
					WHEN c.fact_num < 5000000 AND c.contrib = 1 AND c.iva > 0 AND c.saldo > 0 AND c.anulada = 0 THEN 1
					ELSE 0
				END AS aplica_retencion,
				
				0 AS monto_transito,

				-- Mapeo de tipo (1 para Fiscal, 6 para No Fiscal)
				CASE 
					WHEN c.fact_num >= 5000000 THEN 6
					ELSE 1
				END AS es_nota,
			...
			FROM 
				factura c 
				INNER JOIN clientes cl ON c.co_cli = cl.co_cli
				INNER JOIN docum_cc dcc ON dcc.nro_doc = c.fact_num
			WHERE 
				YEAR(c.fec_emis) = " . intval($anio) . " 
			
				AND c.anulada = '0'
				AND dcc.tipo_doc IN ('FACT', 'N/DB', 'N/CR')    
				AND c.numcon ='' and dcc.numcon ='' 
			ORDER BY 
				prioridad_fiscal ASC, -- 1. Lo fiscalizable va primero
				dcc.tipo_doc ASC,      -- 2. Luego agrupamos por tipo (FACT, N/CR, etc.)
				c.fact_num DESC        -- 3. Finalmente lo más reciente arriba
		";
    ...
    if (count($query) >= 1) {
        ...
        foreach ($query as $r) {
            ...
            $array[$cnt]->nro_control = $r['nro_control'];
  			$array[$cnt]->tipo_documento = $r['es_nota'];
			$array[$cnt]->tipo_doc = $r['tipo_doc'];
            $cnt++;
        }
	...
	public static function GetFacturaCliente($fact_num){
		...
		(p.tot_bruto) as tot_bruto, (p.tot_neto) as tot_neto , (p.iva) as iva,
		 ...
		 t.des_tran,
		 v.ven_des,
		 v.email as correo_vendedor,
		 v.co_ven,
		 p.tasa,
		 z.zon_des,
		 c.campo3,
		 cn.cond_des,
		 p.dir_ent as ent_fact,
		 c.dir_ent2 as ent_cli,
		(p.glob_desc/p.tasa) as glob_desc,
		p.porc_gdesc as porc_desc,
		p.numcon 
		 FROM factura p 
		INNER JOIN almacen a on p.co_sucu = a.co_alma 
		INNER JOIN clientes c on p.co_cli = c.co_cli 
		...
		INNER JOIN zona z on c.co_zon = z.co_zon
		INNER JOIN transpor t ON p.co_tran = t.co_tran
		INNER JOIN condicio cn ON cn.co_cond = p.forma_pag
		WHERE p.fact_num='".$fact_num."'  AND p.status = 0 AND p.co_sucu = '".$co_sucu."' 
		ORDER BY p.fact_num ASC ";  
		...
		foreach($query as $r) {
			...
			$direc1 = trim($r['direc1']);
			$ent_fact = trim($r['ent_fact']);
			$ent_cli = trim($r['ent_cli']);

			if($ent_fact != '' && $ent_fact != null){
				$dir_entrega = $ent_fact;
			}else if($ent_cli != '' && $ent_cli != null){
				$dir_entrega = $ent_cli;
			}else{
				$dir_entrega = $direc1;
			}
			...
			$array[$cnt]->porc_desc = floatval($r['porc_desc'] ?? 0);
			...
			$array[$cnt]->numcon = trim($r['numcon']);	

			$array[$cnt]->telefonos = trim($r['telefonos']);

			$array[$cnt]->direc1 = trim($r['direc1']);	
			// Limpiamos los saltos de línea y espacios extra antes de la asignación
			$dir_entrega_limpia = preg_replace('/\s+/', ' ', trim($dir_entrega));
			// Asignamos el valor formateado en una sola línea horizontal
			$array[$cnt]->ent_fact = $dir_entrega_limpia;
	
			$array[$cnt]->correo_vendedor = trim($r['correo_vendedor']);		
			$array[$cnt]->ven_des = trim($r['ven_des']);		
			$array[$cnt]->co_ven = trim($r['co_ven']);				
			$array[$cnt]->forma_pag = trim($r['cond_des']);	
	...
	public static function getDataFacturasDocumentos($tipo_documento) {
		...
		// 2. Construcción dinámica del filtro según tipo_documento
		$filtro_tipo = "";
		
		switch (intval($tipo_documento)) {
			case 1: // Facturas Fiscales
				$filtro_tipo = "AND dcc.tipo_doc = 'FACT' AND c.fact_num < 500000 AND c.anulada = '0'";
				break;
			case 2: // Notas de Entrega
				$filtro_tipo = "AND dcc.tipo_doc = 'FACT' AND c.fact_num >= 500000 AND c.anulada = '0'";
				break;
			case 3: // Notas de Crédito
				$filtro_tipo = "AND dcc.tipo_doc = 'N/CR' AND c.anulada = '0'";
				break;
			case 4: // Notas de Débito
				$filtro_tipo = "AND dcc.tipo_doc = 'N/DB' AND c.anulada = '0'";
				break;
			case 5: // Anuladas
				$filtro_tipo = " AND c.anulada = '1'";
				break;
			default:
				// Filtro por defecto en caso de error o valor no esperado
				$filtro_tipo = "AND dcc.tipo_doc = 'FACT'";
				break;
		}
	
		// 3. Construimos la consulta SQL
		$sql = "
			SELECT
				c.fact_num AS factura_id,
				CAST(c.fact_num AS VARCHAR) AS fact_num_original,
		...
		if (count($query) >= 1) {
			...
			foreach ($query as $r) {
				...
				// Corrección en el cálculo de saldo_usd si la tasa es 0 para evitar división por cero
				$tasa = (float)$r['tasa'] > 0 ? (float)$r['tasa'] : 1;
				$array[$cnt]->saldo_usd = $r['moneda'] == 'USD' ? (float)$r['saldo'] : (float)$r['saldo_bs'] / $tasa;
				...
				$array[$cnt]->nro_control = $r['nro_control'];
				$array[$cnt]->tipo_documento = $r['es_nota'];
				$array[$cnt]->tipo_doc=intval($tipo_documento);
				$cnt++;
			}
			return $array;
		} else {
    ...
    public function guardarRespuestaApi($fact_num, $numero_control,$tipo_doc) {
        try {
            // Iniciar transacción 
    ...
    public function guardarRespuestaApiNota($fact_num, $numero_control) {
        try {
            // Iniciar transacción 

                // Construir la consulta SQL para cada lote
                $sql = "UPDATE docum_cc  SET numcon = '$numero_control' WHERE nro_doc  = '$fact_num' and  tipo_doc ='FACT'";
	...
	public static function GetFacturaClienteNotaD($fact_num){
		/// Metodo para consultar todos los datos y mostrar las tablas		
	...
	public static function GetRenglonFacturaNotaC($fact_num){
		$co_sucu = $_SESSION['co_alma'];
    ...
    public function guardarRespuestaApiDev($fact_num, $numero_control,$tipo_doc) {
        try {
            // Iniciar transacción 

                // Construir la consulta SQL para cada lote
                $sql = "UPDATE dev_cli  SET numcon = '$numero_control' WHERE fact_num  = '$fact_num' and  tipo_doc ='$tipo_doc'";
    ...
    public function guardarRespuestaApiNotas($fact_num, $numero_control,$tipo_doc) {
        try {
            // Iniciar transacción 
	...
	public static function getDataFacturasNotasCreditoDev($tipo_documento) {
		// 1. Obtenemos el año actual
		$hoy = getdate();
		$anio = $hoy['year'];
	...
	public static function GetFacturaClienteNotaCreditoDev($fact_num){
		/// Metodo para consultar todos los datos y mostrar las tablas		
		
		$co_sucu=$_SESSION['co_alma'];

		$sql ="SELECT p.fact_num as fact_num,   (p.saldo) as saldo, p.fec_emis,p.fec_venc, p.co_tran, p.forma_pag,
		(p.tot_bruto) as tot_bruto, (p.tot_neto) as tot_neto , (p.iva) as iva,
		 p.moneda, p.status, p.contrib,
		 c.cli_des,
		 c.email,
		...
		foreach($query as $r) {
			...
			$array[$cnt]->responsive_id = "";  
			$array[$cnt]->fact_num = $r['fact_num'];
			$array[$cnt]->ncr_num = $r['nc_num'];
			$array[$cnt]->saldo = $r['saldo'];			
			$array[$cnt]->descuento_global = floatval($r['glob_desc'] ?? 0);
			$array[$cnt]->porc_desc = floatval($r['porc_gdesc'] ?? 0);
			$array[$cnt]->fec_emis =$r['fec_emis'];  // abcd ;	
			$array[$cnt]->fec_venc =$r['fec_venc'];  // abcd ;				
			$array[$cnt]->cli_des = trim($r['cli_des']);			
			$array[$cnt]->rif = trim($r['rif']);	
	...
	public static function GetRenglonFacturaClienteNotaCreditoDev($fact_num){
		$co_sucu = $_SESSION['co_alma'];
		...
		$sql = "SELECT 
					...
					pg.porc_desc as descuento
				FROM reng_dvc pg 
				INNER JOIN dev_cli p on p.fact_num=pg.fact_num 
				INNER JOIN art a ON pg.co_art=a.co_art 
				INNER JOIN vendedor v on p.co_ven=v.co_ven 
				INNER JOIN colores c on c.co_col = a.co_color
				WHERE pg.fact_num='".$fact_num."'
	...
	public static function GetRenglonFacturaClienteNotas($fact_num){
		$co_sucu = $_SESSION['co_alma'];
  	...
  	public static function GetFacturaOriginal($fact_num){
		/// Metodo para consultar todos los datos y mostrar las tablas		
		...
		p.fact_num,
		p.fec_emis,
		(p.tot_neto) as tot_neto,
		p.telefono as total_api_facturacion
		 FROM factura p 	
		WHERE p.fact_num='".$fact_num."' 
		ORDER BY p.fact_num ASC ";  
		...
		foreach($query as $r) {
			...
			$array[$cnt]->responsive_id = "";  
			$array[$cnt]->fact_num = $r['fact_num'];			
			$array[$cnt]->tot_neto = floatval($r['tot_neto'] ?? 0);
			$array[$cnt]->total_api_facturacion = floatval($r['total_api_facturacion'] ?? 0);
			$array[$cnt]->fec_emis =$r['fec_emis'];  // abcd ;	
	...
	public static function GetFacturaClienteNotas($fact_num){
		/// Metodo para consultar todos los datos y mostrar las tablas		
				...
				p.forma_pag, 
					...
					p.contrib,
						...
						c.rif,
							...
							v.email as correo_vendedor,	
							  ...
							  c.campo3,
							   ...
							   (p.glob_desc/p.tasa) as glob_desc,
								...
								dcc.monto_imp as iva, 
								dcc.monto_bru as tot_bruto, 
								case when dcc.tipo = '6' then monto_bru  else dcc.monto_otr end as exento,
								dcc.monto_net as tot_neto,
								(dcc.monto_bru - dcc.monto_otr) as monto_base_imponible


							FROM docum_cc dcc
							INNER JOIN factura p on p.fact_num = dcc.nro_orig
							INNER JOIN almacen a on p.co_sucu = a.co_alma 
							INNER JOIN clientes c on p.co_cli = c.co_cli 
							INNER JOIN vendedor v on p.co_ven = v.co_ven 
							INNER JOIN zona z on c.co_zon = z.co_zon
							INNER JOIN transpor t ON p.co_tran = t.co_tran 
							INNER JOIN condicio cn ON cn.co_cond = p.forma_pag 
							WHERE dcc.nro_doc='$fact_num'  and dcc.tipo_doc = 'N/CR' and dcc.aut = 0 and dcc.nro_doc < 5000000 and dcc.anulado=0 AND p.status = 0 AND p.co_sucu = '01' ORDER BY p.fact_num ASC";  
		...
		foreach($query as $r) {
			...
			$array[$cnt]->tot_neto = floatval($r['tot_neto']);	
			$array[$cnt]->iva = floatval($r['iva']);	
			$array[$cnt]->exento = floatval($r['exento']);	
	...
	public static function GetFacturaClienteNotasDebito($fact_num){
		/// Metodo para consultar todos los datos y mostrar las tablas		
				...
				p.forma_pag, 
					...
					p.contrib,
						...
						c.rif,
							...
							INNER JOIN transpor t ON p.co_tran = t.co_tran 
							INNER JOIN condicio cn ON cn.co_cond = p.forma_pag 
							WHERE dcc.nro_doc='$fact_num'  and dcc.tipo_doc = 'N/DB' and dcc.aut = 0 and dcc.nro_doc < 5000000 and dcc.anulado=0 AND p.status = 0 AND p.co_sucu = '01' ORDER BY p.fact_num ASC";  
	...
	public static function getDatosFactura($fact_num){
		/// Metodo para consultar todos los datos y mostrar las tablas		
		...
		foreach($query as $r) {
			...
			$array[$cnt]->porc_gdesc = floatval($r['porc_gdesc'] ?? 0);
			$array[$cnt]->fec_emis =$r['fec_emis'];  // abcd ;	
			$array[$cnt]->fec_venc =$r['fec_venc'];  // abcd ;				
			$array[$cnt]->cli_des = trim($r['cli_des']);			
			$array[$cnt]->rif = trim($r['rif']);	
	...
	public static function getDatosFactura_renglones($fact_num){
		$co_sucu = $_SESSION['co_alma'];
