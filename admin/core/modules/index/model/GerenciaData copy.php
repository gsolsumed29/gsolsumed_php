...
class GerenciaData {
	...
	public  function getAllDatosFacturacion($ano,$mes,$co_ven){
		...
		if($mes == 'all'){
			...
			$filtroMes ="and MONTH(f.fec_emis)='$mes'";
			$filtroMes2= "and  MONTH(d.fec_emis)='$mes' ";
		  ...
		  switch ($co_ven) {
			...
			$sql= "WITH TransaccionesPorVendedor AS (
				...
				FROM factura f
				WHERE  YEAR(f.fec_emis)='$ano' $filtroMes
				AND f.anulada = 0 $ven
				...
				FROM dev_cli d
				WHERE  YEAR(d.fec_emis)='$ano' $filtroMes2
	...
	public static function getAllDatosDevoluciones($ano,$mes,$co_ven){
			...
			if($mes=='all'){
				...
				$filtroMes ="and MONTH(fec_emis)='$mes'";
			}
			...
			$sql = "Select sum(tot_neto/tasa) as monto from dev_cli where  YEAR(fec_emis)='$ano'  $filtroMes AND anulada = 0".$ven;
			//echo $sql;
	...
	public  function getAllDatosCobranzas($ano,$mes,$co_ven){
		  ...
		  switch ($co_ven) {
			...
			if($mes=='all'){
				...
				$filtroMes="and MONTH(r.fec_cheq)='$mes' ";
			}
	...
	public static function getAllDatoTopsArticulos($ano,$mes,$co_ven){
			...
			if($mes=='all'){
				...
				$mesCond = "AND MONTH(f.fec_emis) = '$mes'";
			}
			...
			select TOP 5 a.art_des, SUM(r.total_art - r.total_dev) total_art, a.modelo
							...
							WHERE YEAR(f.fec_emis)  = '$ano' $mesCond
								".$ven."
								AND f.anulada = 0
								AND f.moneda  = 'US$'
								AND a.co_color <> '05'
	...
	public  function getAllDatosClientesNuevos($ano,$mes,$co_ven){
			...
			if($mes=='all'){
				...
				$mesCond ="AND MONTH(f.fec_emis) = '$mes'";
				$mesCond2 ="AND MONTH(c.fecha_reg) = '$mes'";
			}	
	...
	public  function getDatoCalculoImplicito(){
		...
		if($e>=1){
			...
			foreach($query as $r) {			
				...
				$array[$cnt]->dato1 = (float)$r['tasa_implicita'];
	...
	public  function getAllDatosCuentasXPagar($ano,$mes,$co_ven){
		  ...
		  switch ($co_ven) {
			...
			if($mes=='all'){
				...
				$filtroMes="and MONTH(d.fec_venc)='$mes' ";
			}
			...
			$sql = "SELECT 
				...
				WHERE 
					...
					AND YEAR(d.fec_venc) = '$ano' $filtroMes
