...
class RenglonData {
	public static $tablename = "reng_ped";	
		...
		$this->tipo_precio = '';	//pedido
		$this->facturacion = '';	//pedido
		$this->pago = '';	//pedido
	...
	public function add(){
		...
		foreach($data as $carrito){
			...
			$sql="UPDATE st_almac SET stock_com = stock_com + ".$cantidad." WHERE co_art IN ('".$co_art."') AND co_alma IN ('".$this->co_alma."')";
			//echo $sql;
			Executor::doitEx($sql);
			$sql="UPDATE art SET stock_com = stock_com + ".$cantidad."  WHERE co_art IN ('".$co_art."')";
			//echo $sql;
			Executor::doitEx($sql);

		
				$data_articulo=$articulo_objeto->getDataID($carrito['co_art'],1,$this->tipo_precio,$this->facturacion, $this->pago);
				$co_art = $carrito['co_art'];	
				...
				$mont_comi = $data_articulo[0]->mont_comi;
				...
				if($pago=='2'){
					if($mont_comi =='1.00'){
						...
						$data2 = $this->tasa();
						$tasa1=$data2['tasa'];	
						$prec_vta=($precio_dolares*$tasa1);	
						$prec_vta2=($precio_dolares);	
						$ren_neto = ($total_art*$precio_dolares)*$tasa1;
						...
						$data2 = $this->tasa2();
						$tasa2=$data2['tasa'];
						$prec_vta=(($precio_dolares*$tasa2)/$tasa1)*$tasa1;	
						$prec_vta2=(($precio_dolares*$tasa2)/$tasa1);	
						$ren_neto = $total_art*((($precio_dolares*$tasa2)/$tasa1)*$tasa1);
					}
				}else{			
		...
		$prec_vta_factura=number_format((float)$prec_vta, 2, '.', '');
		...
		$sql.=" VALUES('$this->fact_num',$acu,'$co_art','$this->co_alma',$total_art,$total_art,'$uni_venta',$prec_vta_factura,$prec_vta2,$tipo_imp,$ren_neto,$cos_pro_un,$ult_cos_un,cast(getdate() as date),$ult_cos_om)";
		//echo $sql;
		//echo "Dolares o BS.S =>".$this->dato1;
		//echo "<br>";
		//echo "<br>";
		//echo $sql;
		//echo "<br>";
