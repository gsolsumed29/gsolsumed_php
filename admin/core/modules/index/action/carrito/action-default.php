        ...
        if($datos==1){
           ...
           // $qty = 1;
            ...
            $almacen = $_REQUEST['almacen'];
             ...
             $facturacion = $_REQUEST['facturacion']; 
             $pago = $_REQUEST['pago'];
           // echo $co_art."-".$almacen."-".$tipo_precio."-".$facturacion."-".$pago;

            $user_objeto = new ArticuloData();
            $result = $user_objeto->getDataID($co_art,$almacen,$tipo_precio,$facturacion,$pago);
           // var_dump($result);
            $stock_act = $result[0]->stock_act;
            if($qty<=$stock_act){

            /*if($tipo_precio ==3){
                echo "Precio de contado";
             }*/
