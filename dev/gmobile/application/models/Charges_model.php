...
WHERE f.fact_num IN($facturas) GROUP BY f.co_cli,c.cli_des");
    ...
    public function obtener_reng_num($co_cob){
        ...
        public function add_cobro($co_cob,$co_ven,$codcliente,$pagar,$codformapago,$ndocumento,$fecha,$observacion){
            $hoy=date('Y-m-d');
      $data = array(
                    'co_cob' => $co_cob,
                    'co_ven' => $co_ven,
    ...
    public function ver_comprobante($co_cob){
      ...
      $where1=" ";
        ...
        //$fini = date('Y-m-d', strtotime($fini));
        	...
        	$resultado = $this->db->query("SELECT d.co_cob,d.co_cli, c.cli_des, c.rif,d.monto AS saldo, convert(varchar, d.fec_emis, 101) AS fec_emis,convert(varchar, d.fec_pago, 101) AS fec_pago,p.descripcion,d.observacion,d.status,b.des_ban FROM _jm_cobros_tmp d INNER JOIN clientes c ON c.co_cli=d.co_cli $where INNER JOIN _jm_tip_cob p ON p.codigo=d.tipo_pago INNER JOIN bancos b ON b.co_ban=d.co_ban WHERE d.co_ven='$co_ven' AND d.anulada = 0 AND d.tipo='COB' $where1 ORDER BY d.co_cob DESC ");
