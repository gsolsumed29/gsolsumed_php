...
class Rxv extends CI_Controller {
  ...
  public function index(){
    ...
    if($this->session->userdata('user')){
      ...
      $data['lineas'] = $this->rxv_model->get_lineas();
      $data['categorias'] = $this->rxv_model->get_categorias();
      $this->load->view('head');
  ...
  public function getclient()
    {
        ...
        $co_lin = $this->input->post('co_lin');
        $co_cat = $this->input->post('co_cat');
        $list = $this->rxv_model->get_client($nombre,$co_ven,$fini,$ffin,$co_lin,$co_cat);
        $data = array();
        $item=1;
