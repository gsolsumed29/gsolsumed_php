...
class Rxp extends CI_Controller {

  function __construct(){
    parent::__construct();
    $this->load->helper('url');
    $this->load->model('users_model');
    $this->load->model('rxp_model');
  ...
  public function index(){
    ...
    if($this->session->userdata('user')){
      $data['vendedores'] = $this->rxp_model->get_vendedores();
      $this->load->view('head');
      $this->load->view('header');
      $this->load->view('rxp/index',$data);
    }
    else{
      redirect('/');
    }
  ...
  public function getclient()
    {
        ...
        $list = $this->rxp_model->get_client($nombre,$co_ven,$fini,$ffin);
        $data = array();
        $item=1;
