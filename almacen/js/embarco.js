   ...
   // console.log('Buscando paquete con código:', codigo);
    ...
    try {
        ...
        if (Array.isArray(paquetesEncontrados) && paquetesEncontrados.length > 0) {
            ...
            // NUEVO: Guardar en localStorage después de procesar cada paquete
            cargaGuardarPaquetesEnLocalStorage();
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            cargaVerificarFacturasAsociadas();
             ...
             // NUEVO: Actualizar localStorage después de eliminar
            cargaGuardarPaquetesEnLocalStorage();
...
function cargaConfirmarCarga() {
    ...
    // --- CAMBIO 1: Obtener y formatear el ID de la zona ---
    // Obtenemos el array de zonas seleccionadas (para un <select multiple>)
    const zonasSeleccionadas = $('#zona').val(); 
    
    // Validamos que se haya seleccionado al menos una zona
    if (!zonasSeleccionadas || zonasSeleccionadas.length === 0) {
        Swal.fire({
            icon: 'warning',
            title: 'Campo incompleto',
            html: 'Por favor, seleccione al menos una <strong>Zona de despacho</strong>.',
            confirmButtonText: 'Entendido'
        });
        return;
    }
    
    // Unimos los IDs del array en una sola cadena, separados por '*'
    const zonaIdString = zonasSeleccionadas.join('*');
    // --- FIN DEL CAMBIO 1 ---

    // Validar que se hayan seleccionado todos los demás campos necesarios
    if (!vehiculoId || !chofer || !ayudante) {
    ...
    Swal.fire({
        ...
        html: `
            <div style="text-align: left;">
                ...
                <p><strong>Zona de Despacho:</strong> ${$('#zona option:selected').map(function() { return $(this).text(); }).get().join(', ')}</p>
        ...
        if (result.isConfirmed) {
            ...
            // AGREGADO: Obtener la descripción de la zona formateada usando la nueva función
            const zonaDescripcionFormateada = cargaFormatearZonas();

            // 3. Construir el objeto final con encabezado y lotes
            const datosCarga = {
                action: 'confirmar_carga',
                emcabezado: { // Nota: Considera cambiar "emcabezado" a "encabezado" si es posible.
                    vehiculo: vehiculoId,
                    chofer_id: chofer,
                    ayudante_id: ayudante,
                    // --- CAMBIO 2: Usar la cadena formateada ---
                    zona_id: zonaIdString, // Enviamos "Z01*Z03*" en lugar de ["Z01", "Z03"]
                    // --- FIN DEL CAMBIO 2 ---
                    // AGREGADO: Nuevo campo con el texto formateado
                    zona_descripcion: zonaDescripcionFormateada,
...
async function cargaEnviarCarga(datosCarga) {
    try {
        ...
        if (jsonResponse.success) {
             // NUEVO: Limpiar localStorage solo si la carga fue exitosa
            cargaLimpiarLocalStorage();
...
function cargaGuardarPaquetesEnLocalStorage() {
    try {
        localStorage.setItem('cargaPaquetesCargados', JSON.stringify(cargaPaquetesCargados));
        localStorage.setItem('cargaFacturasAsociadas', JSON.stringify(cargaFacturasAsociadas));
...
function cargaCargarPaquetesDesdeLocalStorage() {
    try {
        const paquetesGuardados = localStorage.getItem('cargaPaquetesCargados');
        const facturasGuardadas = localStorage.getItem('cargaFacturasAsociadas');
        
        if (paquetesGuardados) {
            cargaPaquetesCargados = JSON.parse(paquetesGuardados);
            if(cargaPaquetesCargados.length>=1){
                console.log('Paquetes cargados desde localStorage:', cargaPaquetesCargados.length);
            
                // Recrear las filas en la tabla
                $('#filaPaquetes').empty();
                cargaPaquetesCargados.forEach(paquete => {
                    cargaAgregarFilaPaquete(paquete);
                });
                if (facturasGuardadas) {
                    cargaFacturasAsociadas = JSON.parse(facturasGuardadas);
                    console.log('Facturas asociadas cargadas desde localStorage:', cargaFacturasAsociadas.length);
                }
                // Actualizar resumen
                cargaActualizarResumenCarga();
                return cargaPaquetesCargados.length;
            }else{
                return cargaPaquetesCargados.length;
            }
...
function cargaLimpiarLocalStorage() {
    try {
        localStorage.removeItem('cargaPaquetesCargados');
        localStorage.removeItem('cargaFacturasAsociadas');
 ...
 $(document).ready(function() {
    ...
    $('.btnConfirmarCarga').off('click').on('click', function() {
      ...
      if ($('.m_embarco_item').length) {
    // NUEVO: Cargar datos desde localStorage al iniciar
    const hayDatosGuardados = cargaCargarPaquetesDesdeLocalStorage();
    //console.log(hayDatosGuardados);
    if (hayDatosGuardados>=1) {
          Swal.fire({
                icon: 'warning',           
                text: 'Recuerda, Aun tienes lotes embarcados, verifica!'   ,
    ...
    // Y este evento en $(document).ready
 $('#btnLimpiarLocalStorage').on('click', function() {
    Swal.fire({
        title: '¿Limpiar datos guardados?',
        text: "¿Está seguro de que desea eliminar todos los paquetes guardados localmente?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            cargaLimpiarLocalStorage();
            cargaPaquetesCargados = [];
            cargaFacturasAsociadas = [];
            $('#filaPaquetes').empty();
            cargaActualizarResumenCarga();
            
            Swal.fire(
                'Limpiado',
                'Los datos guardados han sido eliminados.',
                'success'
            );
        }
    });
...
// ... (tus funciones existentes como cargaMostrarError) ...

/**
 * AGREGADO: Formatea las zonas seleccionadas en el formato "ID*Texto*ID2*Texto2..."
 * Funciona para selects simples y múltiples.
 * @returns {string} Cadena de texto formateada o cadena vacía si no hay selección.
 */
function cargaFormatearZonas() {
    const $selectedZones = $('#zona option:selected');
    let zonasFormateadas = [];

    // Itera sobre todas las zonas seleccionadas
    $selectedZones.each(function() {
        const valor = $(this).val();
        const texto = $(this).text();
        
        // Asegurarse de que la opción tenga un valor antes de procesarla
        if (valor) {
            zonasFormateadas.push(`${texto}`);
        }
    });

    // Une todas las zonas formateadas con un asterisco
    // Ejemplo: ["5*Zona Norte", "8*Zona Sur"] -> "5*Zona Norte*8*Zona Sur"
    return zonasFormateadas.join('-');
}