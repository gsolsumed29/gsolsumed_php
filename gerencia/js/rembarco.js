...
function cargaInicializarDetallesEmbarque() {
    ...
    if ($('#idEmbarque').length) {
        ...
        if (idEmbarque) {
            ...
            $(document).on('click', '.btnVerDetalleFactura', function() {
                const idFactura = $(this).data('id-factura');
             //   cargaVerDetallesFactura(idFactura);
            });
...
async function cargaDatosEmbarque(idEmbarque) {
    try {
       ...
       // await cargaCargarFacturasEmbarque(idEmbarque);
        ...
        // Cargar ruta del embarque
      //s  await cargaCargarRutaEmbarque(idEmbarque);
        ...
        $('#loadingIndicator').hide();
        cargaMostrarError('Error al cargar los datos del embarque');
    }
}
...
async function cargaCargarInfoGeneralEmbarque(idEmbarque) {
    const params = new URLSearchParams({
        action: 'embarco',
        tipo: '1', // Tipo 2 para obtener detalles
        accion: '2',
        datos: '4',
        c: 'VehiculoData',
        t: 'jm_despacho_carga',
            ...
            lotes.forEach(lote => {
                ...
                const fila = `
                    <tr class="${filaClass}">
                        ...
                        <td>${cargaFormatearEstado(lote.estatus || 'desconocido')}</td>
                        <td>
                            ${botonAccion}
                        </td>
                    </tr>
...
/**
 * Carga las facturas asociadas a un embarque
 * @param {string} idEmbarque - ID del embarque
 */
    ...
    try {
        ...
        if (Array.isArray(facturas) && facturas.length > 0) {
            ...
            facturas.forEach(factura => {
                const fila = `
                    <tr>
                        <td>${factura.fact_num || 'N/A'}</td>
                        <td>${factura.cliente || 'N/A'}</td>
                        <td>${factura.total ? '$' + parseFloat(factura.total).toFixed(2) : '$0.00'}</td>
                        <td>${factura.fec_emis || 'N/A'}</td>
                        <td>${cargaFormatearEstadoFactura(factura.estado || 'desconocido')}</td>
                        <td>
                            <button class="btn btn-sm btn-outline-primary me-1 btnVerDetalleFactura" data-id-factura="${factura.id}">
                                <i data-feather="eye"></i>
                            </button>
                        
                            <button class="btn btn-sm btn-outline-secondary btnGenerarPdfFactura" data-id-factura="${factura.id}">
            ...
            console.warn('No se encontraron facturas para el embarque');
            $('#tablaFacturasEmbarque tbody').append('<tr><td colspan="6" class="text-center">No se encontraron facturas para este embarque</td></tr>');
        }
        
    } catch (error) {
        console.error('Error al cargar facturas del embarque:', error);
        $('#tablaFacturasEmbarque tbody').append('<tr><td colspan="6" class="text-center text-danger">Error al cargar las facturas</td></tr>');
        throw error;
    }
}
 ...
 */
function cargaFormatearEstado(estado) {
...
// =============================================
// MODIFICACIONES AL DOCUMENT READY
// =============================================

 $(document).ready(function() {
    // ... (código existente) ...
    
    // NUEVO: Inicializar sistema de detalles de embarque si estamos en esa vista
