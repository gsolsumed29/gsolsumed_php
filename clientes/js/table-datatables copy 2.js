...
// También mejorar la función procesarFacturas para incluir esta lógica:
function procesarFacturas(arrayFacturas) {
    ...
    // Verificar si el cliente es contribuyente especial
    if (cli_cond == 1) {
        // Esta función ahora verificará si hay facturas aplicables antes de mostrar alerta
        preguntarRegistroRetenciones(arrayFacturas, saldo, saldoBs, co_cli, cli_des);
    } else {
        // Cliente no es contribuyente especial, continuar directamente
...
function validarMontoRetencion(inputElement, montoMaximo) {
    ...
    // Reemplazar la función preguntarRegistroRetenciones con esta versión mejorada:
function preguntarRegistroRetenciones(arrayFacturas, saldo, saldoBs, co_cli, cli_des) {
    // Verificar si hay facturas que aplican para retención
    const facturasAplicanRetencion = arrayFacturas.filter(factura => {
        // Condiciones para aplicar retención:
        // 1. Cliente debe ser agente de retención (cli_cond == 1)
        // 2. Factura debe ser fiscal (tipo_fiscal == 1)
        // 3. NO debe tener retención ya cargada (esto se verifica en otro lugar)
        return factura.cli_cond == 1 && factura.tipo_fiscal === '1';
    });
    
    if (facturasAplicanRetencion.length === 0) {
        // Ninguna factura aplica para retención, continuar directamente
        redireccionarSinRetenciones(arrayFacturas, co_cli, cli_des, saldo, saldoBs);
        return;
    }
    
    // Si hay facturas que aplican, mostrar la alerta
    Swal.fire({
        title: '¿Información?',
        html: `
            <div style="text-align: center;">
...
// Función auxiliar para obtener el tipo de documento legible (opcional, para mejor UX)
function obtenerTipoDocumentoLegible(tipo_fiscal) {
    const tipos = {
        '1': 'Fiscal (con IVA)',
        '6': 'No Fiscal (exenta)',
        '2': 'Consumidor Final',
    ...
    return tipos[tipo_fiscal] || 'Desconocido';
}
  ...
  if (dt_basic_table_clientes_visitas.length) {
    ...
    var dt_adv_filter = dt_basic_table_clientes_visitas.DataTable({
          ...
          render: function (data, type, full, meta) {
            ...
            return (
              `<div class="d-flex justify-content-center col-actions">
                <a class="me-25" href="index.php?view=candidato_1&co_cli=${full.co_cli}&cand_des=${full.cli_des}" 
      ...
      dom: `
        <'row'
          <'col-12 mb-2'
            <'head-label text-center text-md-start'>
          >
        >
        <'row'
          <'col-sm-12 col-md-6 mb-2 mb-md-0'l>
        ...
        <'row'
          ...
          <'col-sm-12 col-md-7'p>
        >
        <'row'<'col-12 mt-2'<'#export-container'>>>
      `,
