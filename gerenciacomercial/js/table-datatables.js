...
function cargarTablaClientes() {
  var datatables_basic_clientes = $('.datatables-basic-clientes');

  if (datatables_basic_clientes.length) {
    // Destruir la instancia anterior si existe para evitar errores de reinicialización
    if ($.fn.dataTable.isDataTable('.datatables-basic-clientes')) {
      datatables_basic_clientes.DataTable().destroy();
    }
    
    let dataClientes = $('.dataClientes').val();
    let arrayClientes = JSON.parse(dataClientes);
    
    // --- MEJORA 1: Crear un Modal para Exportación en Móvil ---
    if ($('#exportModalClientes').length === 0) {
      $('body').append(`
        <div class="modal fade" id="exportModalClientes" tabindex="-1" aria-labelledby="exportModalClientesLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exportModalClientesLabel">Exportar Reporte de Clientes</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body d-flex flex-column">
                <button class="btn btn-outline-primary mb-2 export-btn-clientes" data-export="print">
                  ${feather.icons['printer'].toSvg({ class: 'me-50' })} Imprimir
                </button>
                <button class="btn btn-outline-success mb-2 export-btn-clientes" data-export="excel">
                  ${feather.icons['file'].toSvg({ class: 'me-50' })} Excel
                </button>
                <button class="btn btn-outline-danger mb-2 export-btn-clientes" data-export="pdf">
                  ${feather.icons['clipboard'].toSvg({ class: 'me-50' })} PDF
                </button>
                <button class="btn btn-outline-secondary export-btn-clientes" data-export="copy">
    ...
    // --- MEJORA 2: Crear Modal de Ficha para Detalles del Cliente ---
    if ($('#modalFichaCliente').length === 0) {
      $('body').append(`
        <div class="modal fade" id="modalFichaCliente" tabindex="-1" aria-labelledby="modalFichaClienteLabel" aria-hidden="true">
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="modalFichaClienteLabel">Ficha del Cliente</h5>
                ...
                <div class="row mb-3">
                  <div class="col-md-6">
                    ...
                    <table class="table table-sm table-borderless">
                      <tr><td class="fw-bold" style="width: 40%;">Código:</td><td id="ficha-cliente-codigo">-</td></tr>
                      <tr><td class="fw-bold">Razón Social:</td><td id="ficha-cliente-razon">-</td></tr>
                      <tr><td class="fw-bold">RIF:</td><td id="ficha-cliente-rif">-</td></tr>
                      <tr><td class="fw-bold">Tipo:</td><td id="ficha-cliente-tipo">-</td></tr>
                    </table>
                  </div>
                  <div class="col-md-6">
                    <h6 class="text-primary">Contacto y Ubicación</h6>
                    <table class="table table-sm table-borderless">
                      <tr><td class="fw-bold" style="width: 40%;">Teléfono(s):</td><td id="ficha-cliente-telefonos">-</td></tr>
                      <tr><td class="fw-bold">Email:</td><td id="ficha-cliente-email">-</td></tr>
                      <tr><td class="fw-bold">Dirección:</td><td id="ficha-cliente-direccion">-</td></tr>
    ...
    var dt_basic = datatables_basic_clientes.DataTable({
      ...
      columns: [
        ...
        { data: 'responsive_id' },
        {
          ...
          data: null,
          render: function (data, type, full, meta) {
            // Nombre del cliente como link para la ficha
            const nombre = full['cli_des'] || 'No especificado';
            const rif = full['rif'] || 'N/A';
            return `
              <div>
                <span class="cliente-nombre-link text-primary fw-bold" style="cursor: pointer;">${nombre}</span>
                <div class="text-muted small">${rif}</div>
              </div>
            `;
          }
        },
        {
          ...
          data: null,
          render: function (data, type, full, meta) {
            const telefonos = full['telefonos'] || 'N/A';
            const direccion = full['direc1'] || 'N/A';
            return `
              <div>
                <div class="fw-bold">${telefonos}</div>              
                <div title="${direccion}">${direccion}</div>
              </div>
            `;
          }
        },
        {
          ...
          render: function (data, type, full, meta) {
            // Saldo
            let saldoNumerico = 0;
            let saldoTexto = full['saldo_p'] || '0,00';
            if (saldoTexto && typeof saldoTexto === 'string') {
              saldoNumerico = parseFloat(saldoTexto.replace(/\./g, '').replace(',', '.'));
            }
            const saldoFormateado = '$' + saldoNumerico.toLocaleString('es-ES', { minimumFractionDigits: 2 });
            const saldoBadgeClass = saldoNumerico > 0 ? 'bg-danger' : 'bg-success';

            // Estado
            const estado = full['des_tipo'] || 'Sin definir';
            let estadoBadgeClass = 'bg-secondary';
            if (estado.toLowerCase().includes('activo')) estadoBadgeClass = 'bg-success';
            if (estado.toLowerCase().includes('inactivo')) estadoBadgeClass = 'bg-danger';

            return `
              <div class="d-flex flex-column">
              
                <span class="badge rounded-pill ${estadoBadgeClass}">${estado}</span>
              </div>
            `;
          }
        },
        { data: 'co_cli' }, // Columna oculta para el código
        { data: 'email' }   // Columna oculta para email
      ],
      columnDefs: [
        {
          title: '#',
        ...
        {
          title: 'Cliente',
          // CAMBIO: Se aumenta el ancho para dar más importancia a la Razón Social.
          width: '40%',
          targets: 2,
          visible: true,
          responsivePriority: 1
        ...
        {
          title: 'Contacto y Dirección',
          // CAMBIO: Se reduce el ancho, ya que la información es más compacta.
          width: '40%',
          targets: 3,
          visible: true
        },
        {
          title: 'Saldo y Estado',
          // CAMBIO: Se ajusta el ancho para que el total sea coherente (50% + 25% + 25% = 100%).
          width: '10%',
          targets: 4,
          visible: true
        },
      ...
      order: [[2, 'asc']], // Ordenar por Razón Social por defecto
      dom: '<"card-header border-bottom p-1"<"head-label"><"dt-action-buttons text-end"B>><"d-flex justify-content-between align-items-center mx-0 row"<"col-sm-12 col-md-6"l><"col-sm-12 col-md-6"f>>t<"d-flex justify-content-between mx-0 row"<"col-sm-12 col-md-6"i><"col-sm-12 col-md-6"p>>',
      buttons: [
        {
          extend: 'collection',
          ...
          buttons: [
            {
              ...
              className: 'dropdown-item',
              exportOptions: { columns: ':not(:first-child):not(.control)' },
              title: 'Reporte de Clientes - ' + new Date().toLocaleDateString()
            },
            {
              extend: 'excel',
              text: feather.icons['file'].toSvg({ class: 'font-small-4 me-50' }) + 'Excel',
            ...
            },
            {
              ...
              customize: function (doc) {
                ...
                doc.content[0].text = 'Reporte de Clientes - ' + new Date().toLocaleDateString();
        ...
        },
        {
          ...
          action: function (e, dt, node, config) {
            var exportModal = new bootstrap.Modal(document.getElementById('exportModalClientes'));
            exportModal.show();
          }
        }
      ],
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Detalles del Cliente ' + data.cli_des;
            }
          }),
          renderer: function (api, rowIdx, columns) {
            var data = $.map(columns, function (col, i) {
              if (col.title === '' || col.title === '#' || col.className === 'control') {
                return '';
              }
    ...
    $(dt_basic.table().container()).addClass('clientes-datatable-wrapper');

    // Poner el título
    $('div.head-label').html('<h6 class="mb-0">Listado de Clientes</h6>');

    // --- Lógica para los botones del modal de exportación en móvil ---
    $('.export-btn-clientes').on('click', function() {
      var exportType = $(this).data('export');
      dt_basic.button(0).add(0, exportType).trigger();
      bootstrap.Modal.getInstance(document.getElementById('exportModalClientes')).hide();
    });
    ...
    function mostrarFichaCliente(data) {
      // Saldo
      let saldoNumerico = 0;
      let saldoTexto = data['saldo_p'] || '0,00';
      if (saldoTexto && typeof saldoTexto === 'string') {
        saldoNumerico = parseFloat(saldoTexto.replace(/\./g, '').replace(',', '.'));
      }
      const saldoFormateado = '$' + saldoNumerico.toLocaleString('es-ES', { minimumFractionDigits: 2 });
      ...
      // Estado
      const estado = data['des_tipo'] || 'Sin definir';
      let estadoBadgeClass = 'bg-secondary';
      if (estado.toLowerCase().includes('activo')) estadoBadgeClass = 'bg-success';
      if (estado.toLowerCase().includes('inactivo')) estadoBadgeClass = 'bg-danger';

      // Poblar el modal
      $('#ficha-cliente-codigo').text(data.co_cli || 'N/A');
      $('#ficha-cliente-razon').text(data.cli_des || 'N/A');
      $('#ficha-cliente-rif').text(data.rif || 'N/A');
      $('#ficha-cliente-tipo').html(`<span class="badge rounded-pill ${estadoBadgeClass}">${estado}</span>`);
      $('#ficha-cliente-telefonos').text(data.telefonos || 'N/A');
      $('#ficha-cliente-email').text(data.email || 'N/A');
      $('#ficha-cliente-direccion').text(data.direc1 || 'N/A');
      $('#ficha-cliente-saldo').removeClass('bg-success bg-danger').addClass(saldoBadgeClass).text(saldoFormateado);

      // Guardar datos del cliente en el modal para usarlos en los botones
      $('#modalFichaCliente').data('clienteData', data);

      const modalFicha = new bootstrap.Modal(document.getElementById('modalFichaCliente'));
      modalFicha.show();
    }

    // --- Listener para el nuevo target (.cliente-nombre-link) ---
    datatables_basic_clientes.on('mousedown touchstart', '.cliente-nombre-link', function(e) {
      e.preventDefault();
      const $link = $(this);
      isLongPress = false;
      pressTimer = setTimeout(function() {
        ...
        if (rowData) {
          mostrarFichaCliente(rowData);
        }
      }, 500);
    });

    datatables_basic_clientes.on('mouseup touchend mouseleave', '.cliente-nombre-link', function(e) {
    ...
    $('#ficha-boton-editar').on('click', function() {
      const data = $('#modalFichaCliente').data('clienteData');
      if (data) {
        const clienteId = data.co_cli;
        Swal.fire({
          icon: 'info',
          title: 'Función no implementada',
          text: `Aquí abrirías el formulario para editar al cliente ${data.cli_des} (ID: ${clienteId}).`,
    ...
    $('#ficha-boton-ver-saldo').on('click', function() {
      const data = $('#modalFichaCliente').data('clienteData');
      if (data) {
        const clienteId = data.co_cli;
        Swal.fire({
          icon: 'info',
          title: 'Función no implementada',
          text: `Aquí podrías cargar un reporte detallado del saldo para ${data.cli_des} (ID: ${clienteId}).`,
          confirmButtonText: 'Entendido',
          confirmButtonColor: '#2c9aff'
        });
      }
...
function cargarTablaArticulos(){
  const dt_basic_table_articulos = $('.datatables-basic-articulos');   
      ...
      const columnasJS = 10; // Ahora son 10 columnas con la nueva
  ...
  // Crear botón switch en el header
  const switchButton = `
    <div class="btn-group btn-group-toggle price-toggle" data-toggle="buttons">
      <label class="btn btn-primary active">
        <input type="radio" name="price-options" id="price-usd" autocomplete="off" checked>
        <i class="fas fa-dollar-sign"></i> $       </label>
      <label class="btn btn-outline-primary">
        <input type="radio" name="price-options" id="price-bsd" autocomplete="off">
        <i class="fas fa-bolt"></i> BS.D
      </label>
    </div>
  `;
  ...
  var dt_basic = dt_basic_table_articulos.DataTable({
    ...
    columns: [    
      ...
      { data: 'art_des' },       // 2
      { data: 'stock_act' },     // 3    
      { data: 'des_col' },       // 4 - NUEVA COLUMNA AQUÍ
      { data: 'prec_vta1' },     // 5 - Contado USD
      { data: 'prec_vta2' },     // 6 - Crédito USD
      { data: 'prec_vta3' },     // 7 - Contado BS.D
      { data: 'prec_vta4' },     // 8 - Crédito BS.D
      { data: 'tipo_impuesto_desc' } // 9 - Impuesto
  ],
  columnDefs: [
      {
          // For Responsive
      ...
      },  
      {
          targets: 2,
          title: 'Descripcion',
          width: '30%' // Reducido para la nueva columna
      }, 
      {
          targets: 3,
          title: 'En stock',
          ...
          render: function (data, type, full, meta) {
              ...
              if(stock === 0){
                  return '<span class="badge rounded-pill badge-light-danger">Agotado</span>';
              }else{
                  return `<strong >${stock.toFixed(2)}</strong>`;
              }
          }
      },
      {
          targets: 4, // NUEVA DEFINICIÓN PARA LA COLUMNA des_col
          title: 'Marca',
          width: '10%',
          render: function (data, type, full, meta) {
              const desCol = full['des_col'] || '';
              return `<span class="text-wrap">${desCol}</span>`;
          }
      },
      {
          // Precio en USD Contado
          targets: 5, // Cambiado de 4 a 5
          title: 'Contado USD', // Corregí el título para que coincida con el tipo de precio
          width: '8%',
          className: 'price-usd-contado-column',
          render: function (data, type, full, meta) {
              const precio = parseFloat(full['prec_vta1'] || 0);
              const formattedPrice = formatVenezuelanCurrency(precio, 'USD');
              
              return `<strong >${formattedPrice}</strong>`;
          }
      },
      {
          // Precio en USD Crédito
          targets: 6, // Cambiado de 5 a 6
          title: 'Crédito USD', // Corregí el título
          width: '8%',
          className: 'price-usd-credito-column',
          render: function (data, type, full, meta) {
      ...
      {
          // Precio en BS.D Contado
          targets: 7, // Cambiado de 6 a 7
          title: 'Contado BS.D', // Corregí el título
          width: '8%',
          className: 'price-bsd-contado-column',
          render: function (data, type, full, meta) {
              const precio = parseFloat(full['prec_vta3'] || 0);
              const formattedPrice = formatVenezuelanCurrency(precio, 'VES');
              
               return `<strong >${formattedPrice}</strong>`;
          }
      },
      {
          // Precio en BS.D Crédito
          targets: 8, // Cambiado de 7 a 8
          title: 'Crédito BS.D', // Corregí el título
          width: '8%',
          className: 'price-bsd-credito-column',
          render: function (data, type, full, meta) {
      ...
      {
          // Impuesto
          targets: 9, // Cambiado de 8 a 9
          title: 'Impuesto',
          width: '8%',
          render: function (data, type, full, meta) {
              const impuesto = full['tipo_impuesto_desc'] || '';
              if(impuesto === 'G'){
                  return '<span class="badge rounded-pill badge-light-danger">G</span>';
              }else{
                  return `<span class="badge rounded-pill badge-light-success">E</span>`;
              }
          }
      }
    ...
    buttons: [   
      {
          ...
          className: 'btn btn-relief-danger btnGenerarListaPDF',
          action: function (e, dt, node, config) {
          ...
          generarListaPDFCompleta(tipoMoneda, datosFiltrados, searchTerm);
           //console.log(tipoMoneda, datosFiltrados, searchTerm);
        },
          init: function (api, node, config) {
              $(node).removeClass('btn-secondary');
          }
    ...
    responsive: {
      details: {
        ...
        renderer: function (api, rowIdx, columns) {
          // 1. Verificar el estado del switch al momento de renderizar el modal
          const showUSD = $('#price-usd').is(':checked');

          const data = columns.map(col => {
            // 2. Si la columna no tiene título (como la de control), la ignoramos.
            if (col.title === '') {
              return '';
            }

            // 3. Identificar las columnas de precios por su índice
            const isUSDColumn = col.columnIndex === 5 || col.columnIndex === 6; // Indices de precios USD
            const isBSDColumn = col.columnIndex === 7 || col.columnIndex === 8; // Indices de precios BS.D

            // 4. Aplicar la lógica de filtrado
            // Si se muestra USD y la columna es de BS.D, no la renderizamos.
            if (showUSD && isBSDColumn) {
              return '';
            }
            // Si se muestra BS.D y la columna es de USD, no la renderizamos.
            if (!showUSD && isUSDColumn) {
              return '';
            }
            
            // 5. Si la columna pasa el filtro, la renderizamos.
            return `
    ...
    initComplete: function() {
      // Guardar referencia a la DataTable usando this.api()
      var table = this.api();
      
      // Añadir el botón switch al header de la tabla
      $('.head-label').html(`
        <div class="price-toggle-container">
          <span class="me-2">Moneda:</span>
          ${switchButton}
        </div>
      `);
...
function cargarTablaClientesVisitas(data){
...
function cargarTablaFacturasCliente() {
  ...
  if (datatables_basic_facturas.length) {
    ...
    orderable: false,
    render: function (data, type, full, meta) {
        // Obtener datos relevantes para las condiciones
        const tipoFiscal = full['dato9'] || '';          // tipo_fiscal de la factura
        const esAgenteRetencion = full['dato7'] == 1;    // si el cliente es agente (dato7 = cli_cond)
        const tieneIVA = tipoFiscal == 1;                // factura con IVA (tipo_fiscal = 1)
        const montoTransito = parseFloat(full['monto_transito'] || 0);
        const retencionCargada = full['dato11'] != 0;    // Verificar si ya tiene retención cargada
        
        // Condición completa: debe aplicar retención Y cliente debe ser agente Y factura debe tener IVA
        // Y NO debe tener una retención ya cargada (dato11 != 0)
        const aplicaRetencion = tipoFiscal == 1 && esAgenteRetencion && tieneIVA && !retencionCargada;
        
        // Si aplica retención, mostrar el icono para retención sin depósito
        if (aplicaRetencion) {
            return (
                '<div class="d-inline-flex">' +
        ...
        // Si ya tiene retención cargada, mostrar icono de verificación (check)
        if (retencionCargada) {
        ...
        // Si no aplica, mostrar espacio vacío o icono deshabilitado
        return (
            '<div class="d-inline-flex">' +
  ...
  $(document).on('change', '.dt-checkboxes.facturas:not(:disabled)', function() {
    ...
    if(groupRow.length) {
      ...
      // Manejar el evento click para retención sin depósito por fila
    $(document).on('click', '.btn-retencion-sin-deposito', function() {
        ...
        // Verificar si la factura aplica para retención
        ...
        // Mostrar modal para esta factura
...
function procesarRetencionIndividual() {
    ...
    formData.append('datos', '4'); // Código para retenciones sin depósito
...
/// Función separada para procesar los datos
/*function procesarDatosRetencion(factura, numRetencion, montoBs, tasaRetencion, fechaRetencion, archivo) {
    ...
    const retencion = {
        ...
        numRetencion: numRetencion,
        montoBs: montoBs,
        tasaRetencion: tasaRetencion, // Guardamos el porcentaje
        fechaRetencion: fechaRetencion,
        co_cli: factura.co_cli,
        cli_des: factura.cli_des,
        tipo_fiscal: factura.tipo_fiscal,
        tasa_configurada: parseFloat(factura.cli_condm) || 75 // Guardamos la tasa configurada
    };
    
    // Crear FormData para enviar también el archivo si existe
    const formData = new FormData();
    formData.append('datos', JSON.stringify(retencion));
    ...
    // Enviar por AJAX
    $.ajax({
         url: '../admin/index.php?action=pagos', 
        url: 'ruta/a/tu/endpoint', // Reemplaza con tu endpoint real
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        success: function(response) {
            ...
            if (response.success) {
                Swal.fire({
                    icon: 'success',
                    title: 'Retención procesada',
                    text: 'La retención se ha registrado correctamente.'
                }).then(() => {
                    // Cerrar el modal y recargar o actualizar la interfaz según sea necesario
                    $('#modalRetencionSinDeposito').modal('hide');
                    
                    // Aquí puedes recargar la tabla o la interfaz si es necesario
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: response.message || 'Ocurrió un error al procesar la retención.'
                });
            }
        },
...
// Función auxiliar para validar el monto de retención
function validarMontoRetencion(inputElement, montoMaximo) {
    ...
    if (value > montoMaximo) {
        Swal.fire({
            ...
            text: `El monto de retención no puede exceder ${montoMaximo.toFixed(2)} BS.`
        });
        return false;
    }
   ...
   function obtenerDatosFacturaPorId(facturaId) {
    try {
        ...
        return {
            ...
            montoImp: parseFloat(dataAttributes['montoimp' + facturaId] || 0), // Nuevo campo
            co_cli: dataAttributes['cocli' + facturaId] || '',
            cli_des: dataAttributes['clides' + facturaId] || '',
            cli_cond: dataAttributes['clicond' + facturaId] || '',
            cli_condm: dataAttributes['clicondm' + facturaId] || '75', // Valor por defecto 75%
...
function generarModalRetencionIndividual(factura) {
    ...
    // Usar montoImp como base para el cálculo en lugar de montoBs
    const montoBase = parseFloat(factura.montoImp) || 0;
    const montoMaximo = parseFloat(factura.montoBs) || 0; // Límite máximo
    
    // Obtener la tasa de retención específica del cliente desde data-clicondm
    const tasaRetencionCliente = parseFloat(factura.cli_condm) || 0;
    const tasaRetencion = tasaRetencionCliente > 0 ? tasaRetencionCliente / 100 : 0.75; // Convertir porcentaje a decimal
    ...
    $('#procesarRetencionSinDepositoBtn').off('click').on('click', function() {
      ...
      // Función para calcular el monto basado en la tasa cambiada
    function calcularMontoDesdeTasa(inputTasa, montoBase, montoMaximo) {
        const tasa = parseFloat(inputTasa.value) / 100; // Convertir porcentaje a decimal
        const montoCalculado = montoBase * tasa;
        
        // Asegurar que no exceda el máximo permitido
        const montoFinal = Math.min(montoCalculado, montoMaximo);
        
        // Actualizar el campo de monto
        $('.monto-retencion-bs-sd').val(montoFinal.toFixed(2));
        
        // Si se excede el máximo, ajustar la tasa mostrada
        if (montoCalculado > montoMaximo) {
            const tasaAjustada = (montoMaximo / montoBase) * 100;
            inputTasa.value = tasaAjustada.toFixed(2);
            mostrarErrorMonto(`¡Atención! El monto calculado excedía el máximo permitido. Se ajustó a ${montoMaximo.toFixed(2)} BS (tasa ${tasaAjustada.toFixed(2)}%)`);
        } else {
            ocultarErrorMonto();
        }
        
        // Validar el monto
        validarMontoRetencion($('.monto-retencion-bs-sd')[0], montoMaximo);
    }

// Función para calcular la tasa basada en el monto cambiado
function calcularTasaDesdeMonto(inputMonto, montoBase, montoMaximo) {
    const monto = parseFloat(inputMonto.value);
    
    // Asegurar que no exceda el máximo permitido
    const montoAjustado = Math.min(monto, montoMaximo);
    if (monto !== montoAjustado) {
        inputMonto.value = montoAjustado.toFixed(2);
        monto = montoAjustado;
        mostrarErrorMonto(`¡Atención! El monto excedía el máximo permitido. Se ajustó a ${montoMaximo.toFixed(2)} BS`);
    } else {
        ocultarErrorMonto();
    }
    
    const tasaCalculada = (monto / montoBase) * 100; // Convertir a porcentaje
    
    // Actualizar el campo de tasa
    $('.tasa_retencion_sd').val(tasaCalculada.toFixed(2));
    
    // Validar el monto
    validarMontoRetencion(inputMonto, montoMaximo);
}

    // Función para calcular la tasa basada en el monto cambiado
    function calcularTasaDesdeMonto(inputMonto, montoMaximo) {
        const monto = parseFloat(inputMonto.value);
        const tasaCalculada = (monto / montoMaximo) * 100; // Convertir a porcentaje
    ...
    function validarMontoRetencion(input, maximo) {
    const valor = parseFloat(input.value);
    
    if (isNaN(valor) || valor <= 0) {
        input.setCustomValidity('El monto debe ser mayor a 0');
        mostrarErrorMonto('El monto debe ser mayor a 0');
        return false;
    }
    
    if (valor > maximo) {
        input.setCustomValidity(`El monto no puede exceder ${maximo.toFixed(2)} BS`);
        mostrarErrorMonto(`El monto no puede exceder ${maximo.toFixed(2)} BS`);
        return false;
    }
    
    // Validar coherencia con la tasa configurada del cliente
    const tasaCliente = parseFloat($('.tasa_retencion_sd').val()) || 0;
    const tasaCalculada = (valor / maximo) * 100;
    const diferencia = Math.abs(tasaCalculada - tasaCliente);
    
    if (diferencia > 5) { // Más del 5% de diferencia
        input.setCustomValidity(`El monto representa una tasa de ${tasaCalculada.toFixed(2)}%, que difiere significativamente de la tasa configurada (${tasaCliente}%)`);
        mostrarErrorMonto(`¡Atención! La tasa calculada (${tasaCalculada.toFixed(2)}%) difiere de la configurada (${tasaCliente}%)`);
        return false;
    }
    ...
    function mostrarErrorMonto(mensaje) {
        let $errorDiv = $('.error-monto-retencion');
        if ($errorDiv.length === 0) {
            $errorDiv = $('<div class="alert alert-danger mt-2 error-monto-retencion"></div>');
            $('.monto-retencion-bs-sd').closest('.col-md-6').append($errorDiv);
        }
        $errorDiv.html(`<small>${mensaje}</small>`).show();
    }

    function ocultarErrorMonto() {
        $('.error-monto-retencion').hide();
    }
  }
}
  ...
  if (dt_basic_table_clientes_visitas.length) {
    ...
    $('.export-btn').on('click', function() {
      ...
      function mostrarFichaCandidato(data) {
          ...
          const visitUrl = `index.php?view=candidato_1&co_cli=${data.co_cli}&cand_des=${encodeURIComponent(data.cli_des)}`;
...
$(window,document,$).ready(function(){
  ...
  cargarDataClientesCandidatos('NO');
...
function validarFormatos_candidatos() {
    ...
    if (contactoTelefono && !validarFormatoTelefono(contactoTelefono)) {
        errors.push('El <b>teléfono de contacto</b> debe contener solo números (máximo 10 dígitos)');
    }
...
if ($('.m_visitas_clientes').length) {
  resaltarMenu('.i_visitas_clientes');
}
...
function cargarDataClientes(){
  ...
  $.ajax({
    type: "GET",
    url: '../admin/index.php?action=cliente&c=ClienteData&a=1&t=clientes&tipo=1&accion=9&datos=1&filtro=1', 
...
function cargarDataClientesVisitas(){
  ...
  $('.dataClientesVisitas').attr("value",data);
  cargarTablaClientesVisitas(data);
...
function cargarDataClientesCandidatos($filtro){
  if ($('#dataClientesCandidatos').length) {
  $.ajax({
    type: "GET",
    url: '../admin/index.php?action=cliente&tipo=1&accion=10&datos=2&c=ClienteData&a=1&t=clientes&filtro='+$filtro, 
}).done(function(datos) { 
