...
function generarBotonesAccionFiscales(full, tipoTab) {
    ...
    if ([1, 3, 4, 5].includes(parseInt(tipoTab)) && full.url_fiscal) {
        html += `<button class="btn btn-sm btn-icon btn-relief-success btn-ver-documento-tfhka" 
                  ...
                  html += `<button class="btn btn-sm btn-icon btn-relief-info btn-descargar-documento-tfhka" 
                    data-num_control="${full.fact_num}" 
                    title="Descargar">
...
$(document).on('click', '.btn-descargar-documento-tfhka', function(e) {
    e.preventDefault();
    
    const btn = $(this);
    const numControl = btn.attr('data-num_control');
    const factNum = btn.attr('data-fact_num');
    
    // Validar que tengamos el número de control/documento
    if (!numControl && !factNum) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'No se encontró el número de control del documento.',
            confirmButtonColor: '#ea5455'
        });
        return;
    }
    
    // El numeroDocumento según el DTO debe ser solo dígitos (1-19)
    const numeroDocumento = numControl || factNum;
    
    // Obtener datos completos de la fila
    const tabla = $('.tabla-maestra-fiscalizados').DataTable();
    const fila = tabla.row(btn.closest('tr')).data();
    
    // Determinar el tipo de documento
    const tipoTabActual = parseInt($('.tab-fiscal-link.active').attr('data-tipo-doc')) || 1;
    
    // Mapeo de tipoTab a tipoDocumento fiscal (según DTO: 01-07)
    const mapeoTipoDocumento = {
        1: '01', // Factura
        2: '02', // Nota de Entrega
        3: '03', // Nota de Crédito por Devolución
        4: '02', // Nota de Crédito Administrativa
        5: '03'  // Nota de Débito
    };
    
    const tipoDocumento = mapeoTipoDocumento[tipoTabActual] || '01';
    
    // Obtener serie si existe en los datos
    const serie = (fila && fila.serie) ? fila.serie : null;
    
    // Menú de selección de formato
    Swal.fire({
        title: 'Descargar Documento Fiscal',
        html: `
            <div style="margin-bottom: 15px;">
                <p><strong>Documento:</strong> ${factNum || numeroDocumento}</p>
                <p><strong>Tipo:</strong> ${tipoDocumento}</p>
                ${serie ? `<p><strong>Serie:</strong> ${serie}</p>` : ''}
            </div>
            <p style="margin-bottom: 10px; font-weight: bold;">Seleccione el formato:</p>
            <div style="display: flex; gap: 10px; justify-content: center;">
                <button type="button" class="btn btn-outline-primary btn-formato-descarga" 
                        data-formato="pdf" style="flex: 1;">
                    <i data-feather="file-text"></i><br>
                    <strong>PDF</strong><br>
                    <small>Documento visual</small>
                </button>
                         </div>
        `,
        showCancelButton: true,
    ...
    $(document).one('click', '.btn-formato-descarga', function() {
        const formato = $(this).data('formato');
        Swal.close();
        descargarDocumentoFiscal(serie, tipoDocumento, numeroDocumento.toString(), formato);
    });
});
...
/**
 * Descarga un documento fiscal desde TFHKA
 * Basado en DownloadRequestDTO
 * 
 * @param {string|null} serie - Serie del documento (opcional, max 20 chars)
 * @param {string} tipoDocumento - Tipo de documento (01-07) *REQUERIDO*
 * @param {string} numeroDocumento - Número del documento (1-19 dígitos) *REQUERIDO*
 * @param {string} tipoArchivo - Formato: pdf, xml, json (opcional)
 */

/**
/**
 * Descarga un documento fiscal desde TFHKA
 * Mismo patrón que anulación
 */
function descargarDocumentoFiscal(serie, tipoDocumento, numeroDocumento, tipoArchivo = 'pdf') {
    
    // ========== VALIDACIONES DEL DTO ==========
    if (!tipoDocumento || !/^(01|02|03|04|05|06|07)$/.test(tipoDocumento)) {
        Swal.fire({
            icon: 'error',
            title: 'Error de validación',
            text: 'El tipo de documento debe ser un valor entre 01 y 07.',
            confirmButtonColor: '#ea5455'
        });
        return;
    }
    
    if (!numeroDocumento || !/^[0-9]{1,19}$/.test(numeroDocumento.toString())) {
        Swal.fire({
            icon: 'error',
            title: 'Error de validación',
            text: 'El número de documento debe contener solo dígitos (1-19 caracteres).',
            confirmButtonColor: '#ea5455'
        });
        return;
    }
    ...
    if (serie && serie.length > 20) {
        Swal.fire({
            icon: 'error',
            title: 'Error de validación',
            text: 'La serie no puede exceder los 20 caracteres.',
            confirmButtonColor: '#ea5455'
        });
        return;
    }
    
    if (tipoArchivo && !/^(pdf|PDF|xml|XML|json|JSON)$/.test(tipoArchivo)) {
        tipoArchivo = 'pdf';
    }
    ...
    // ========== CONSTRUIR PAYLOAD ==========
    const payload = {
        serie: (serie && serie.trim() !== '') ? serie.trim().substring(0, 20) : null,
        tipoDocumento: tipoDocumento,
        numeroDocumento: numeroDocumento.toString(),
        tipoArchivo: (tipoArchivo && tipoArchivo.trim() !== '') ? tipoArchivo.toLowerCase() : null
    };

    console.log('📤 Payload de descarga:', JSON.stringify(payload, null, 2));
    ...
    Swal.fire({
        ...
        html: `
            <div style="text-align: left;">
                <p><b>Tipo:</b> ${payload.tipoDocumento}</p>
                <p><b>Número:</b> ${payload.numeroDocumento}</p>
                ${payload.serie ? `<p><b>Serie:</b> ${payload.serie}</p>` : ''}
                <p><b>Formato:</b> ${payload.tipoArchivo?.toUpperCase() || 'PDF'}</p>
            </div>
        `,
        allowOutsideClick: false,
        didOpen: () => {
    ...
    // ========== PETICIÓN AJAX (mismo patrón que anulación) ==========
    $.ajax({
        url: '../admin/index.php?action=facturacion&c=FacturaData&tipo=7&accion=4&datos=1&t=factura',
        type: 'POST',
        ...
        success: function(response) {
            ...
            // ✅ Misma verificación que en anulación
            if (response.status === 'success' && response.data) {
                
                // Buscar el archivo en la respuesta
                const archivo = response.data.archivo || 
                                response.data.resultado?.archivo;
                
                if (archivo) {
                    const nombreArchivo = `documento_${numeroDocumento}.${tipoArchivo}`;
                    descargarArchivoBase64(archivo, nombreArchivo, tipoArchivo);
                    
                    Swal.fire({
                        icon: 'success',
                        title: '¡Descarga exitosa!',
                        text: response.data.mensaje || response.message || 'El documento se ha descargado correctamente.',
                        timer: 2500,
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'No se encontró el archivo en la respuesta del servidor.',
                        confirmButtonColor: '#ea5455'
                    });
                ...
                // Error (mismo patrón que anulación)
                const mensajeError = response.message || response.data?.mensaje || 'Error al descargar el documento';
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: mensajeError,
        ...
        error: function(xhr, status, error) {
            ...
            let errorMsg = 'No se pudo conectar con el servidor de descarga.';
            ...
            Swal.fire({
                ...
                text: errorMsg,
                footer: `Código de estado: ${xhr.status}`,
                confirmButtonColor: '#ea5455'
            });
 ...
 */
function descargarArchivoBase64(base64Data, nombreArchivo, tipoArchivo) {
    try {
        let base64Limpio = base64Data;
        ...
        console.error('Error al procesar el archivo base64:', error);
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'No se pudo procesar el archivo descargado.',
            confirmButtonColor: '#ea5455'
        });
    }
}
...
/**
 * Descarga contenido directo como archivo
 */
function descargarContenido(contenido, numeroDocumento, tipoArchivo) {
    try {
        const mimeTypes = {
            'pdf': 'application/pdf',
            'xml': 'application/xml',
        ...
        const mimeType = mimeTypes[tipoArchivo] || 'text/plain';
        const blob = new Blob([contenido], { type: mimeType });
        
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `documento_${numeroDocumento}.${tipoArchivo}`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
        ...
        Swal.fire({
            icon: 'success',
            title: '¡Descarga exitosa!',
            text: `El archivo ${tipoArchivo.toUpperCase()} se ha descargado correctamente.`,
            timer: 2000,
            showConfirmButton: false
        });
    } catch (error) {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'No se pudo crear el archivo para descargar.',
            confirmButtonColor: '#ea5455'
        });
    }
}
...
/**
 * Procesa errores en la respuesta del servidor
 */
function procesarErrorRespuesta(response) {
    const mensaje = response.mensaje || response.message || 'Error desconocido del servidor';
    const errores = response.validaciones || response.errors || [];
    
    let htmlError = `<p>${mensaje}</p>`;
    if (errores.length > 0) {
 ...
 */
function procesarErrorConexion(xhr, status, error) {
    let errorMsg = 'No se pudo conectar con el servidor.';
    let detalles = '';
    
    if (xhr.responseJSON) {
        errorMsg = xhr.responseJSON.message || xhr.responseJSON.mensaje || errorMsg;
        if (xhr.responseJSON.validaciones) {
            detalles = '<br>' + xhr.responseJSON.validaciones.join('<br>');
        }
    } else if (xhr.responseText) {
        try {
            const response = JSON.parse(xhr.responseText);
            errorMsg = response.message || response.mensaje || errorMsg;
        } catch (e) {
    ...
    Swal.fire({
        icon: 'error',
        title: 'Error de conexión',
        html: `${errorMsg}${detalles}`,
        footer: `Estado HTTP: ${xhr.status} ${xhr.statusText || ''}`,
        confirmButtonColor: '#ea5455'
    });
